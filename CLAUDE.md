...
- `sbt g8Test` — renders the template into `target/sbt-test/…` using `default.properties` values and
  runs `sbt test` inside the generated project. This is the full end-to-end check and the only real
  compile verification. `sbt test` is aliased to `g8Test` in the root `build.sbt`.
- Requires network access: `default.properties` resolves most library versions dynamically from
  Maven (`maven(org.typelevel,cats-core_3,stable)`), so a rendered project pulls whatever is current.
...
the dependency version pins — add a new library by adding a `*_version` entry there, referencing it
